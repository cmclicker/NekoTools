...
| Phase 2   | **Active**                  | NekoEnv shipped (PR #12/#13/#14; free tier closed at `a442233`). NekoLogs charter merged (PR #15 / `03b5853`); NekoLogs engine MVP + `@nekotools/lens-kit` extraction in review (this PR); NekoLogs UI is the queued next-up implementation. |
...
| 14    | Phase 2 / NekoLogs 2.x.1 | Implementation | `@nekotools/lens-logs` engine MVP: `log.text` (JSON-per-line / logfmt / plaintext detection) + `log.filter` parsers, diagnostics, summary + basic histogram, text/messages/json/csv/markdown exporters, workspace round-trip + conformance tests. **Extracts `@nekotools/lens-kit`** (clock + id-factory) and re-points lens-binary/json/env/logs at it — the 3rd-reuse trigger from NekoJSON charter §7. No UI. | In review (this PR) |
