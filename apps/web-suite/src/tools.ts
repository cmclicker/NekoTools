 ...
 *
 * The shell renders the tab strip and the Free / Pro entitlement
 * surface from this list, so adding a tool to those surfaces is a
 * one-line registration here. (Each tool still mounts its own panel in
 * `App.tsx`, because the sub-apps have bespoke props / test seams; new
 * tools add a `TOOLS` entry plus their panel together.)
 */
