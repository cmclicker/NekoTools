...
/**
 * Build a NekoNDJSON registration for the runtime. Free exporters run for
 * everyone. The Pro exporters (inferred JSON Schema + CSV flatten) are
 * registered as `proExporters` and gated by `runExporter` behind a valid
 * entitlement (single-build-gated model, same as NekoJSON / NekoTOML). The
 * filter-records, stream-large, schema-validation, and workspace-snapshot Pro
